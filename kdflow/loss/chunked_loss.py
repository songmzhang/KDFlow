...
from kdflow.metrics import accumulate_metric_stats
...
def chunked_loss(
    ...
    **kwargs: Any,
    ...
    Metric functions return {name: (sum, count)}; these statistics are never normalized here.
    """
    ...
    for start in range(0, student_hidden.shape[0], chunk_size):
        end = min(start + chunk_size, student_hidden.shape[0])
        ...
        if metric_fns is not None and has_teacher_logits:
            ...
            for fn in fns:
                ...
                accumulate_metric_stats(metric_stats, metrics)
        if reduction == "none":
